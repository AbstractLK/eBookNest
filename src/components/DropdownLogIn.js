...
export const DropdownLogIn = ({setShowMenu}) => {
    ...
    useEffect(() => {
        async function fetchUserData(){
            try {
                ...
                data.email ? setUserData(data) : handleLogout();                
            } catch (error) {
    ...
    return (
      <div id="dropdownAvatar" className="select-none	absolute top-10 right-0 z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
          ...
          <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownUserAvatarButton">
              ...
              <li>
              <Link onClick={() => setShowMenu(false)} to="/dashboard" className="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Dashboard</Link>
              </li>
          </ul>
          <div className="py-1">
